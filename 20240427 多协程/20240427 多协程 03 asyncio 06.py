...
# @Author   : Shawn
# @Version  : V 1.0
# @Fime     : 20240427 多协程 03 asyncio 06.py
# @Desc     :
...
async def main() -> None:
    ...
    tasks = [
        create_task(names(data_faker, AMOUNT_DATA, WAIT_MIN, WAIT_MAX, DELAY_DEFAULT), name="Tom"),
        create_task(numbers(data_faker, AMOUNT_DATA, WAIT_MIN, WAIT_MAX, DELAY_DEFAULT), name="Jerry"),
    ]

    done, pending = await wait(tasks, timeout=None)
    for task in done:
